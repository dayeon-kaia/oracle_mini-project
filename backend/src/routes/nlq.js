...
const generateMockLactateData = (limit) => {
...
const generateMockStayData = (stayId, limit) => {
...
const generateMockPatientList = (limit) => {
  ...
  rows.sort((a, b) => Number.parseFloat(b.risk_mortality_24h) - Number.parseFloat(a.risk_mortality_24h));
...
router.post("/", async (req, res) => {
  try {
    ...
    if (question.toLowerCase().includes("lactate")) {
      mockData = generateMockLactateData(Math.min(safeLimit, 30));
      ...
      mockData = generateMockStayData(stayId, Math.min(safeLimit, 24));
      sql = `SELECT stay_id, charttime, hr, map, spo2, temp\nFROM vitals\nWHERE stay_id = ${stayId}\n  AND charttime >= NOW() - INTERVAL '24 hours'\nORDER BY charttime DESC\nLIMIT ${safeLimit};`;
    } else {
      mockData = generateMockPatientList(Math.min(safeLimit, 50));
    ...
    return res.json({
      ...
      warnings: mockData.rows.length >= safeLimit ? [`Result set truncated to ${safeLimit} rows`] : [],
      logs: [
        {
          step: "received_question",
          status: "success",
